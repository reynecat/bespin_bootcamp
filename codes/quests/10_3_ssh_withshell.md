...
content=$(cat ./server_logs.txt)


echo "전체 로그 라인 수:"
echo "$content" | wc -l

echo "$content" | grep "ERROR" > ./errors.log


error_count=$(echo "$content" | grep -c "ERROR")
warning_count=$(echo "$content" | grep -c "WARNING")
info_count=$(echo "$content" | grep -c "INFO")
...
if [ -s ./errors.log ]; then
    ...
    cat ./errors.log | cut -d ']' -f 2- | cut -d ':' -f 1 | sort | uniq -c | sort -nr | head -1
fi


total_lines=$(echo "$content" | wc -l)
if [ $total_lines -gt 0 ]; then
    error_percentage=$(awk "BEGIN {printf \"%.0f\", ($error_count/$total_lines)*100}")

    echo "ERROR 비율: $error_percentage%"

    if [ $error_percentage -ge 30 ]; then
        echo "위험"
    elif [ $error_percentage -ge 20 ]; then
        echo "주의"
    else
        echo "정상"
    fi
...
[ohjimin@192.168.12.129 ~/shell_practice]$ source log_monitor.sh
전체 로그 라인 수:
12
ERROR 개수: 5
WARNING 개수: 2
...
[ohjimin@192.168.12.129 ~/shell_practice]$ cat server_logs.txt
2024-01-15 10:30:15 INFO User login successful: user001
2024-01-15 10:31:22 ERROR Database connection failed
2024-01-15 10:32:05 INFO User login successful: user002
2024-01-15 10:33:18 WARNING Memory usage high: 85%
