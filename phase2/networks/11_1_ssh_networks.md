...
total_count=$(echo "$content" | wc -l)
success_count=$(echo "$content" | grep -c "success")
failed_count=$(echo "$content" | grep -c "failed")
success_percentage=$(awk "BEGIN {printf \"%.0f\", ($success_count/$total_count)*100}")

echo "=== 네트워크 연결 분석 결과 ===  "

echo "전체 연결 시도: $total_count"
...
content=$(cat ./network.log)

echo "$content" | sort -k 3 | echo > "$
